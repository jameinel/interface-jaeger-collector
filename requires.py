...
class JaegerCollectorClient(RelationBase):
    # Existing Jaeger Collector client interface listed here:
    # ????
    scope = scopes.UNIT
    auto_accessors = ['host', 'port']

    @hook('{requires:jaeger-collector}-relation-{joined,changed}')
    def changed(self):
    ...
    @hook('{requires:jaeger-collector}-relation-{departed,broken}')
    def departed(self):
        conv = self.conversation()
    ...
    def list_unit_data(self):
        ''' Iterate through all Jaeger Collector conversations and return the data
        for each cached conversation. This allows us to build a cluster string
        directly from the relation data. eg:

        for unit in jaegercollector.list_data():
