...
class JaegerCollectorProvides(RelationBase):
    # Every unit connecting will get the same information
    scope = scopes.GLOBAL

    # Use some template magic to declare our relation(s)

    @hook('{provides:jaeger-collector}-relation-{joined,changed}')
    def joined(self):
        conv = self.conversation()
        conv.set_state('{relation_name}.connected')

    @hook('{provides:jaeger-collector}-relation-{departed,broken}')
    def departed(self):
        conv = self.conversation()
